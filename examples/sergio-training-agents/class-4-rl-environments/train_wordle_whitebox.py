...
"""
White-box demo for Training Agents Class 4: GRPO + OpenEnv's Wordle environment.
The trainer drives every step through environment_factory (the white-box hookup).
Based on examples/scripts/openenv/wordle.py from TRL, with two changes:
- works with async OpenEnv clients (EnvClient.reset/step return coroutines now)
- pushes the trained model to the Hub with a clean name
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Hub repo for the trained model (e.g. user/qwen3-1.7b-wordle-grpo).",
    )
    parser.add_argument(
        "--vllm-server-url",
        type=str,
...
def main() -> None:
    ...
    class WordleEnv:
        ...
        def _close_session(self):
            # Closing on done keeps the server session count bounded; without
            # this the server accumulates sessions until CAPACITY_REACHED.
            try:
                self.client.close()
            except Exception:
                pass
        ...
        def reset(self, **kwargs) -> str | None:
            # TRL pools env instances and reuses them across generation batches,
            # so a client closed after a finished game is recreated here.
            # .sync() wraps the async client with a dedicated background event
            # loop, so the websocket stays bound to one loop.
            if self.client is None:
                # Generous message timeout: the server answers slowly while many
                # sessions reset back to back, and a starved client loop can fire
                # the timer late even after the response arrived.
                self.client = TextArenaEnv(base_url=env_url, message_timeout_s=300).sync()
    ...
    output_dir = args.output_dir or f"{args.model.split('/')[-1].lower()}-wordle-grpo"
