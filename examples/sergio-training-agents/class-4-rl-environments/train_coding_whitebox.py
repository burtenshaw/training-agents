...
"""
White-box coding demo for Training Agents Class 4: GRPO + OpenEnv's coding environment.

The model solves MBPP problems inside a live Python session served by an OpenEnv
Space. The `run_python` tool executes code in the session (state persists across
calls), and the environment owns the reward: `get_reward` runs the task's hidden
tests in the same session and returns the fraction that pass. No reward_funcs
needed, the trainer picks up `get_reward` automatically.
...
Sizing note: the generation batch (per_device_train_batch_size x
gradient_accumulation_steps) equals the number of live websocket sessions held
against the env Space at once, and
`prompts per step = per_device_train_batch_size x gradient_accumulation_steps / num_generations`.
The defaults give 64 sessions and 16 prompts per step. Keep the generation batch
modest or the Space runs out of sessions.

If you want to watch the model improve, use `train_coding_whitebox_eval.py`
instead: with only a handful of problems per step, this script's `reward` curve is
dominated by which problems were drawn, not by the model.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run GRPO training on MBPP using the OpenEnv coding environment."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Number of MBPP problems to train on (the sanitized train split has 120). Note the default is "
        "120, so pass it explicitly if you concatenate more splits.",
    )
    parser.add_argument(
        "--num-generations",
        type=int,
