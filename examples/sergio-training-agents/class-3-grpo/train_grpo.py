...
"""
GRPO on MBPP with a verifiable reward: the completion's code is executed against the
problem's own unit tests, and the reward is the fraction of tests that pass.

Usage (num_generations, the group size, is the knob to play with):
...
python train_grpo.py \
    ...
    --output_dir qwen3-0.6b-mbpp-grpo
"""
...
def tests_reward(completions, test_list, test_setup_code, **kwargs):
    """Fraction of the problem's asserts that pass when run against the completion's code.

    Each assert runs in a subprocess with a timeout: generated code can loop forever.
    This executes model-generated code with no sandbox, which is fine inside a
    disposable job container; use a real sandbox beyond a demo (e.g. Hugging Face
    Sandboxes: huggingface.co/docs/huggingface_hub/guides/sandbox).
    """
    rewards = []
